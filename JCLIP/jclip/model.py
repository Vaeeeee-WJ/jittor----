...
class CLIP(nn.Module):
    ...
    def execute(self, image, text):
        ...
        # return logits_per_image, logits_per_text, text_features, image_features  #TAG  no_finetune_v1 
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        self.conv1 = nn.Conv(inplanes, planes, 1, stride=1, bias=False)
        self.bn1 = nn.BatchNorm(planes)
        self.relu1 = nn.ReLU()

        self.conv2 = nn.Conv(planes, planes, 3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm(planes)
        self.relu2 = nn.ReLU()

        self.avgpool = nn.AvgPool2d(stride) if stride > 1 else nn.Identity()

        self.conv3 = nn.Conv(planes, planes * self.expansion, 1, stride=1, bias=False)
...
class AttentionPool2d(nn.Module):
    def __init__(self, spacial_dim, embed_dim, num_heads, output_dim=None):
        super().__init__()
        self.positional_embedding = jt.array(jt.init.gauss((spacial_dim ** 2 + 1, embed_dim)) / embed_dim ** 0.5)
    ...
    def execute(self, x):
        n, c, h, w = x.shape
        x = x.view(n, c, h * w).permute(2, 0, 1)  # NCHW -> (HW)NC
        cls_token = x.mean(dim=0, keepdims=True)  # Calculate the mean of x and keep the dimension
        x = jt.concat([cls_token, x], dim=0)  # (HW+1)NC
        x = x + self.positional_embedding.unsqueeze(1).broadcast(x.shape)  # (HW+1)NC
        ...
        q = q.view(q.shape[0], q.shape[1], self.num_heads, -1).permute(2, 1, 0, 3)  # (HW+1)NC -> (HW+1)N(num_heads)C//num_heads
        k = k.view(k.shape[0], k.shape[1], self.num_heads, -1).permute(2, 1, 0, 3)
        v = v.view(v.shape[0], v.shape[1], self.num_heads, -1).permute(2, 1, 0, 3)

        # Scaled dot-product attention
        attn_weights = jt.matmul(q, k.transpose(-1, -2)) / (q.shape[-1] ** 0.5)  # (HW+1)N(num_heads)C//num_heads
        attn_weights = nn.softmax(attn_weights, dim=-1)
        attn_output = jt.matmul(attn_weights, v)  # (HW+1)N(num_heads)C//num_heads

        # Concatenate multiple heads and project the output
        attn_output = attn_output.permute(2, 1, 0, 3).view(attn_output.shape[2], attn_output.shape[1], -1)
